...
<?php
  $getID = $_GET["id"];
  $sql="select * from tbl_tahun where id_tahun='$getID' ";
  $q=mysql_query($sql);
...
?>
  ...
  <form class="form-horizontal" role="form" method="POST" action="">
            ...
            <input name="id_kompetisi" type="hidden" value="<?php echo $id_tahun ?>" class="form-control" readonly="readonly">
        ...
        Tahun Penilaian</br>
            <div class="col-md-4">
            <input name="nama_kompetisi" type="text" value="<?php echo $tahun ?>" class="form-control" required="required">
            </div> </div>
        </br>
          <div class="col-md-2"></div>
			<div class="form-group">
                <button type="submit" class="btn btn-info">Ubah</button>
                <a href="?menu=tahunpenilaian"><button type="button" class="btn btn-primary">Batal</button></a>
              </div>
    </fieldset>
  </form>
...
<?php // simpan
  if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	$qcekstatus=mysql_query("select * from tbl_tahun where status='1' and id_tahun!='$getID'");
	$jumlahstat = mysql_num_rows($qcekstatus);
	if($jumlahstat==0){

      $sqlsimpan = "UPDATE tbl_tahun SET
                    id_tahun = '$id_kompetisi',
                    tahun = '$nama_kompetisi',
					status = '$statuskomp'
